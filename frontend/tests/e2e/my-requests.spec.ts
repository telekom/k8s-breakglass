...
// This test file uses ops-user-gamma (TEST_USERS.opsUser) who has group "monitoring-team"
// It targets the "ui-e2e-my-requests-test" escalation which only allows "monitoring-team" group
...
test.describe.serial("My Pending Requests View", () => {
  test("user can view my requests page", async ({ page }) => {
    ...
    // Use ops-user-gamma who has monitoring-team group (isolated from other tests)
    await auth.loginViaKeycloak(TEST_USERS.opsUser);
  ...
  test("user can withdraw pending request", async ({ page }) => {
    ...
    if (escalationCard) {
      ...
      if (await pendingCard.isVisible()) {
        ...
        // Either we get a success toast OR the card changes state
        await Promise.race([
        ...
        // If there are still pending cards, make sure none of them has our test reason
        // This is a softer check that accounts for timing issues
        if (count > 0) {
