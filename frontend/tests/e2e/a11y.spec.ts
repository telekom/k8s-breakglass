...
/**
 * Axe rules disabled because they are triggered entirely by Scale's internal
  ...
  const significantViolations = results.violations
    ...
    .map((v) => ({
      ...
      nodes: v.nodes.filter((n) => {
        ...
        // Exempt Telekom brand colours from contrast rules (AA + AAA)
        if ((v.id === "color-contrast-enhanced" || v.id === "color-contrast") && isBrandColorContrastNode(n))
          return false;
        return true;
      }),
    }))
