...
export default async function getConfig(): Promise<Config> {
  ...
  if (data.oidcAuthority && data.oidcClientID) {
    return { oidcAuthority: data.oidcAuthority, oidcClientID: data.oidcClientID };
  }
  if (data.frontend && data.frontend.oidcAuthority && data.frontend.oidcClientID) {
    return { oidcAuthority: data.frontend.oidcAuthority, oidcClientID: data.frontend.oidcClientID };
  }
  logError('ConfigService', 'Config missing OIDC fields', data);
  return { oidcAuthority: "", oidcClientID: "" };
}
