...
assert_count() {
  ...
  # awk returns zero for an empty result, avoiding grep's version-dependent
  # exit status while still checking the rendered Kubernetes objects.
...
assert_count "${rendered}" '^kind: DebugPodTemplate$' 3 "default pod template count"
assert_count "${rendered}" '^kind: DebugSessionTemplate$' 3 "default session template count"
...
# Check the order of the first (pod-template) set only. Helm renders the
# matching session-template set afterwards; selecting every other annotation
# would make this dependent on whether a Helm release inserts extra objects.
actual_intents="$(awk '
  /^kind: DebugSessionTemplate$/ { exit }
  /catalogue-intent:/ {
    value = $0
    sub(/^.*catalogue-intent:[[:space:]]*"/, "", value)
    sub(/".*$/, "", value)
    print value
  }
' "${all_rendered}" | paste -sd, -)"
...
for profile in workload-diagnostics storage-diagnostics cluster-validation; do
  ...
  if [[ "${profile}" != cluster-validation ]]; then
    assert_block_contains "${block}" 'automountServiceAccountToken: false' "${profile} must not receive a service account token"
  fi
