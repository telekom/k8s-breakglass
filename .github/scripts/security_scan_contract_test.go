...
func TestCatalogueDispatchRevisionIsConsistent(t *testing.T) {
	_, source, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("locate workflow contract test")
	}
	data, err := os.ReadFile(filepath.Join(filepath.Dir(source), "../workflows/catalogue-utility-integration.yml"))
	if err != nil {
		t.Fatal(err)
	}
	var workflow struct {
	...
	if len(workflow.Jobs) == 0 {
		t.Fatal("missing catalogue jobs")
	}
	for name, job := range workflow.Jobs {
		...
		for _, step := range job.Steps {
			if strings.HasPrefix(step.Uses, "actions/checkout@") {
				found = true
				if step.With["ref"] != "${{ github.event.inputs.ref || github.event.pull_request.head.sha || github.sha }}" {
					t.Errorf("job %s must test the requested dispatch revision or PR head", name)
				}
			}
		}
		if !found {
			t.Errorf("job %s has no source checkout", name)
		}
	}
}
