...
// TestValidateOIDCFromIdentityProviderConfig_ClientSecretRefMissingFields tests clientSecretRef validation
func TestValidateOIDCFromIdentityProviderConfig_ClientSecretRefMissingFields(t *testing.T) {
	tests := []struct {
		name      string
		...
		{
			name: "clientSecretRef missing name",
			cfg: &OIDCFromIdentityProviderConfig{
				Name:   "my-idp",
				Server: "https://api.cluster.example.com:6443",
				ClientSecretRef: &SecretKeyReference{
			...
			expectErr: "clientSecretRef.name",
		},
		{
			name: "clientSecretRef missing namespace",
			cfg: &OIDCFromIdentityProviderConfig{
				Name:   "my-idp",
				Server: "https://api.cluster.example.com:6443",
				ClientSecretRef: &SecretKeyReference{
			...
			expectErr: "clientSecretRef.namespace",
		},
	}
...
// TestValidateOIDCFromIdentityProviderConfig_CASecretRefMissingFields tests caSecretRef validation
func TestValidateOIDCFromIdentityProviderConfig_CASecretRefMissingFields(t *testing.T) {
	tests := []struct {
		name      string
		cfg       *OIDCFromIdentityProviderConfig
		expectErr string
	}{
		{
			...
			expectErr: "caSecretRef.name",
		},
		{
			name: "caSecretRef missing namespace",
			cfg: &OIDCFromIdentityProviderConfig{
				Name:   "my-idp",
				Server: "https://api.cluster.example.com:6443",
				CASecretRef: &SecretKeyReference{
			...
			expectErr: "caSecretRef.namespace",
		},
	}
...
// TestValidateOIDCFromIdentityProviderConfig_InvalidServerURL tests invalid server URLs
func TestValidateOIDCFromIdentityProviderConfig_InvalidServerURL(t *testing.T) {
	cfg := &OIDCFromIdentityProviderConfig{
		Name:   "my-idp",
		Server: "http://insecure.example.com", // HTTP not HTTPS
	}
	errs := validateOIDCFromIdentityProviderConfig(cfg, field.NewPath("spec", "oidcFromIdentityProvider"))
	assert.NotEmpty(t, errs, "HTTP server URL should fail validation")
	errStr := errs.ToAggregate().Error()
	assert.Contains(t, errStr, "https")
}

// TestValidateOIDCFromIdentityProviderConfig_NilConfig tests nil config handling
func TestValidateOIDCFromIdentityProviderConfig_NilConfig(t *testing.T) {
	errs := validateOIDCFromIdentityProviderConfig(nil, field.NewPath("spec", "oidcFromIdentityProvider"))
	assert.Empty(t, errs, "nil config should pass validation")
}

// TestValidateOIDCAuthConfig_CASecretRefMissingFields tests caSecretRef validation in OIDCAuthConfig
func TestValidateOIDCAuthConfig_CASecretRefMissingFields(t *testing.T) {
	tests := []struct {
		name      string
		oidc      *OIDCAuthConfig
		expectErr string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			errs := validateOIDCAuthConfig(tt.oidc, field.NewPath("spec", "oidcAuth"))
			assert.NotEmpty(t, errs, "missing caSecretRef fields should fail validation")
			errStr := errs.ToAggregate().Error()
			assert.Contains(t, errStr, tt.expectErr)
		})
	}
...
// TestValidateOIDCAuthConfig_TokenExchangeSubjectTokenEmptyName tests empty name in SubjectTokenSecretRef
func TestValidateOIDCAuthConfig_TokenExchangeSubjectTokenEmptyName(t *testing.T) {
	oidc := &OIDCAuthConfig{
		IssuerURL: "https://keycloak.example.com/realms/test",
		ClientID:  "breakglass",
		Server:    "https://api.cluster.example.com:6443",
	...
	errs := validateOIDCAuthConfig(oidc, field.NewPath("spec", "oidcAuth"))
	assert.NotEmpty(t, errs, "empty subjectTokenSecretRef.name should fail validation")
	errStr := errs.ToAggregate().Error()
	assert.Contains(t, errStr, "subjectTokenSecretRef")
	assert.Contains(t, errStr, "name")
}

// TestValidateOIDCAuthConfig_TokenExchangeActorTokenEmptyName tests empty name in ActorTokenSecretRef
func TestValidateOIDCAuthConfig_TokenExchangeActorTokenEmptyName(t *testing.T) {
	oidc := &OIDCAuthConfig{
		IssuerURL: "https://keycloak.example.com/realms/test",
		ClientID:  "breakglass",
		Server:    "https://api.cluster.example.com:6443",
	...
	errs := validateOIDCAuthConfig(oidc, field.NewPath("spec", "oidcAuth"))
	assert.NotEmpty(t, errs, "empty actorTokenSecretRef.name should fail validation")
	errStr := errs.ToAggregate().Error()
	assert.Contains(t, errStr, "actorTokenSecretRef")
}

// TestValidateOIDCAuthConfig_TokenExchangeMissingClientSecretRef tests token exchange requires clientSecretRef
func TestValidateOIDCAuthConfig_TokenExchangeMissingClientSecretRef(t *testing.T) {
	oidc := &OIDCAuthConfig{
		IssuerURL: "https://keycloak.example.com/realms/test",
		ClientID:  "breakglass",
		Server:    "https://api.cluster.example.com:6443",
		// Missing ClientSecretRef
		TokenExchange: &TokenExchangeConfig{
			Enabled: true,
			SubjectTokenSecretRef: &SecretKeyReference{
				Name:      "subject-token",
	...
	errs := validateOIDCAuthConfig(oidc, field.NewPath("spec", "oidcAuth"))
	assert.NotEmpty(t, errs, "token exchange without clientSecretRef should fail")
	errStr := errs.ToAggregate().Error()
	assert.Contains(t, errStr, "clientSecretRef")
	assert.Contains(t, errStr, "required when token exchange")
}
...
func TestValidateOIDCAuthConfig_NilConfig(t *testing.T) {
	errs := validateOIDCAuthConfig(nil, field.NewPath("spec", "oidcAuth"))
	assert.Empty(t, errs, "nil config should pass validation")
}

// TestValidateOIDCAuthConfig_DuplicateScopes tests duplicate scopes detection
func TestValidateOIDCAuthConfig_DuplicateScopes(t *testing.T) {
	oidc := &OIDCAuthConfig{
		IssuerURL: "https://keycloak.example.com/realms/test",
		ClientID:  "breakglass",
		Server:    "https://api.cluster.example.com:6443",
		Scopes:    []string{"openid", "groups", "openid"}, // Duplicate
	}
	errs := validateOIDCAuthConfig(oidc, field.NewPath("spec", "oidcAuth"))
