...
type DebugSessionAuditConfig struct {
	...
	// +kubebuilder:validation:Pattern=`^[0-9]+(ns|us|µs|ms|s|m|h|d|w|y)([0-9]+(ns|us|µs|ms|s|m|h|d|w|y))*$`
...
func validateDebugSessionTemplateSpec(template *DebugSessionTemplate) field.ErrorList {
	...
	if (mode == DebugSessionModeWorkload || mode == DebugSessionModeHybrid) && !hasPodTemplateRef && !hasPodTemplateString {
		allErrs = append(allErrs, field.Required(specPath.Child("podTemplateRef"),
