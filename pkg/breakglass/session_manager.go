...
func NewSessionManager(contextName string) (SessionManager, error) {
	...
	if err != nil {
		log.Errorw("Failed to get config with context", "context", contextName, "error", err.Error())
	...
	if err != nil {
		log.Errorw("Failed to create new client", "error", err.Error())
...
func NewSessionManagerWithClientAndReader(c client.Client, reader client.Reader, log ...*zap.SugaredLogger) SessionManager {
	if reader == nil {
		reader = c
	}
	return SessionManager{Client: c, reader: reader, log: getLoggerOrDefault(log...)}
}

func (c SessionManager) getLog() *zap.SugaredLogger {
	if c.log != nil {
		return c.log
	}
	return nopLogger
...
func (c SessionManager) GetAllBreakglassSessions(ctx context.Context) ([]v1alpha1.BreakglassSession, error) {
	c.getLog().Debug("Fetching all BreakglassSessions")
	cgal := v1alpha1.BreakglassSessionList{}
	if err := c.list(ctx, &cgal); err != nil {
		c.getLog().Errorw("Failed to get BreakglassSessionList", "error", err.Error())
		return nil, fmt.Errorf("failed to get BreakglassSessionList: %w", err)
	}
	c.getLog().Infow("Fetched BreakglassSessions", "count", len(cgal.Items))
...
func (c SessionManager) GetSessionsByState(ctx context.Context,
	...
	c.getLog().Debugw("Fetching BreakglassSessions by state (using field index)", "state", state)
	...
	if err := c.Client.List(ctx, &bsl, client.MatchingFields{"status.state": string(state)}); err != nil {
		if !isFieldIndexError(err) {
			...
			c.getLog().Errorw("Failed to list BreakglassSessions by state", "state", state, "error", err)
		...
		c.getLog().Debugw("Field index not available; falling back to client-side filtering", "state", state, "error", err)
	...
	c.getLog().Infow("Fetched BreakglassSessions by state (indexed)", "count", len(bsl.Items), "state", state)
...
func (c SessionManager) GetBreakglassSessionByName(ctx context.Context, name string) (v1alpha1.BreakglassSession, error) {
	...
	c.getLog().Debugw("Fetching BreakglassSession by name (direct GET)", system.NamespacedFields(name, "")...)
	...
	if err := c.Get(ctx, client.ObjectKey{Name: name}, &bs); err == nil {
		c.getLog().Infow("Fetched BreakglassSession by name (direct GET)", system.NamespacedFields(name, bs.Namespace)...)
		return bs, nil
	} else {
		c.getLog().Debugw("Direct GET failed; falling back to list across namespaces", "name", name, "error", err.Error())
	}
	...
	if err := c.List(ctx, &indexed, client.MatchingFields{"metadata.name": name}); err == nil {
		...
		case 0:
			c.getLog().Debugw("Field index lookup returned no sessions; falling back to selector", "name", name)
		case 1:
			found := indexed.Items[0]
			c.getLog().Infow("Fetched BreakglassSession by name (field index)", system.NamespacedFields(found.Name, found.Namespace)...)
			return found, nil
		default:
			...
			c.getLog().Errorw("Ambiguous BreakglassSession name across namespaces (field index)", "name", name, "namespaces", namespaces)
			return bs, fmt.Errorf("%s", msg)
		}
	} else {
		c.getLog().Debugw("Field index lookup failed; falling back to selector", "name", name, "error", err.Error())
	}
	...
	if ferr != nil {
		c.getLog().Errorw("Failed to parse field selector for fallback lookup", "selector", selector, "error", ferr.Error())
	...
	if err := c.list(ctx, &bsl, &client.ListOptions{FieldSelector: fs}); err != nil {
		c.getLog().Errorw("Failed to list BreakglassSessionList for fallback lookup", "selector", selector, "error", err.Error())
		return bs, fmt.Errorf("failed to list BreakglassSessionList for fallback lookup: %w", err)
	}

	if len(bsl.Items) == 0 {
		c.getLog().Debugw("BreakglassSession not found by name across namespaces", "name", name)
	...
	if len(bsl.Items) > 1 {
		...
		msg := fmt.Sprintf("multiple BreakglassSessions with name %q found in namespaces: %s", name, strings.Join(namespaces, ","))
		c.getLog().Errorw("Ambiguous BreakglassSession name across namespaces", "name", name, "namespaces", namespaces)
		return bs, fmt.Errorf("%s", msg)
	}

	// Single match: return it
	found := bsl.Items[0]
	c.getLog().Infow("Fetched BreakglassSession by name (fallback list)", system.NamespacedFields(found.Name, found.Namespace)...)
	return found, nil
}
...
func (c SessionManager) GetUserBreakglassSessions(ctx context.Context,
	...
	c.getLog().Debugw("Fetching BreakglassSessions for user (using field index)", "user", user)
	bsl := v1alpha1.BreakglassSessionList{}
	// Use the cached client (c.Client.List) for indexed queries.
	// Field indexes are only available in the cache, not via APIReader.
	if err := c.Client.List(ctx, &bsl, client.MatchingFields{"spec.user": user}); err != nil {
		if !isFieldIndexError(err) {
			c.getLog().Errorw("Failed to list BreakglassSessions for user", "user", user, "error", err)
			return nil, fmt.Errorf("failed to list BreakglassSessions for user: %w", err)
		}
		// Field index not available — fall back to client-side filtering.
		c.getLog().Debugw("Field index not available; falling back to client-side filtering", "user", user, "error", err)
		all, err := c.GetAllBreakglassSessions(ctx)
		if err != nil {
			return nil, err
		}
	...
	c.getLog().Infow("Fetched BreakglassSessions for user (indexed)", "count", len(bsl.Items), "user", user)
	return bsl.Items, nil
}
...
func (c SessionManager) GetClusterUserBreakglassSessions(ctx context.Context,
	...
	c.getLog().Debugw("Fetching BreakglassSessions for cluster and user (using field index)", "cluster", cluster, "user", user)
	bsl := v1alpha1.BreakglassSessionList{}
	// Use the cached client (c.Client.List) for indexed queries.
	// Field indexes are only available in the cache, not via APIReader.
	if err := c.Client.List(ctx, &bsl, client.MatchingFields{"spec.cluster": cluster, "spec.user": user}); err != nil {
		if !isFieldIndexError(err) {
			c.getLog().Errorw("Failed to list BreakglassSessions for cluster/user", "cluster", cluster, "user", user, "error", err)
			return nil, fmt.Errorf("failed to list BreakglassSessions for cluster/user: %w", err)
		}
		// Field index not available — fall back to client-side filtering.
		c.getLog().Debugw("Field index not available; falling back to client-side filtering", "cluster", cluster, "user", user, "error", err)
		all, err := c.GetAllBreakglassSessions(ctx)
		if err != nil {
			return nil, err
		}
	...
	c.getLog().Infow("Fetched BreakglassSessions (indexed)", "count", len(bsl.Items), "cluster", cluster, "user", user)
	return bsl.Items, nil
}
...
func (c SessionManager) GetBreakglassSessionsWithSelectorString(ctx context.Context,
	...
	c.getLog().Debugw("Fetching BreakglassSessions with selector string", "selector", selectorString)
	bsl := v1alpha1.BreakglassSessionList{}

	fs, err := fields.ParseSelector(selectorString)
	if err != nil {
		c.getLog().Errorw("Failed to create field selector", "selector", selectorString, "error", err)
		return nil, fmt.Errorf("failed to create field selector %q : %w", selectorString, err)
	}

	if err := c.list(ctx, &bsl, &client.ListOptions{FieldSelector: fs}); err != nil {
		c.getLog().Errorw("Failed to list BreakglassSessionList with string selector", "selector", selectorString, "error", err.Error())
		return nil, fmt.Errorf("failed to list BreakglassSessionList with string selector: %w", err)
	}
	c.getLog().Infow("Fetched BreakglassSessions with selector string", "count", len(bsl.Items), "selector", selectorString)
	return bsl.Items, nil
}
...
func (c SessionManager) GetBreakglassSessionsWithSelector(ctx context.Context,
	...
	c.getLog().Debugw("Fetching BreakglassSessions with selector", "selector", fs.String())
	bsl := v1alpha1.BreakglassSessionList{}

	if err := c.list(ctx, &bsl, &client.ListOptions{FieldSelector: fs}); err != nil {
		c.getLog().Errorw("Failed to list BreakglassSessionList with selector", "selector", fs.String(), "error", err.Error())
		return nil, fmt.Errorf("failed to list BreakglassSessionList with selector: %w", err)
	}
	c.getLog().Infow("Fetched BreakglassSessions with selector", "count", len(bsl.Items), "selector", fs.String())
	return bsl.Items, nil
}
...
func (c SessionManager) AddBreakglassSession(ctx context.Context, bs *v1alpha1.BreakglassSession) error {
	c.getLog().Infow("Adding new BreakglassSession", append(system.NamespacedFields(bs.Name, bs.Namespace), "user", bs.Spec.User, "cluster", bs.Spec.Cluster)...)
	if err := c.Create(ctx, bs); err != nil {
		c.getLog().Errorw("Failed to create new BreakglassSession", append(system.NamespacedFields(bs.Name, bs.Namespace), "error", err.Error())...)
		return fmt.Errorf("failed to create new BreakglassSession: %w", err)
	}
	c.getLog().Infow("BreakglassSession created successfully", system.NamespacedFields(bs.Name, bs.Namespace)...)
...
func (c SessionManager) UpdateBreakglassSession(ctx context.Context, bs v1alpha1.BreakglassSession) error {
	c.getLog().Infow("Updating BreakglassSession", system.NamespacedFields(bs.Name, bs.Namespace)...)
	...
	if err := utils.ApplyObject(ctx, c.Client, &bs); err != nil {
		c.getLog().Errorw("Failed to update BreakglassSession", append(system.NamespacedFields(bs.Name, bs.Namespace), "error", err.Error())...)
		return fmt.Errorf("failed to update BreakglassSession %s/%s: %w", bs.Namespace, bs.Name, err)
	}
	c.getLog().Infow("BreakglassSession updated successfully", system.NamespacedFields(bs.Name, bs.Namespace)...)
...
func (c SessionManager) UpdateBreakglassSessionStatus(ctx context.Context, bs v1alpha1.BreakglassSession) error {
	c.getLog().Infow("Updating BreakglassSession status", system.NamespacedFields(bs.Name, bs.Namespace)...)
	...
	if err != nil {
		c.getLog().Errorw("Failed to resolve BreakglassSession before status update", append(system.NamespacedFields(bs.Name, bs.Namespace), "error", err.Error())...)
	...
	if err := applyBreakglassSessionStatus(ctx, c, &bs); err != nil {
		c.getLog().Errorw("Failed to update BreakglassSession status", append(system.NamespacedFields(bs.Name, bs.Namespace), "error", err.Error())...)
		return fmt.Errorf("failed to update BreakglassSession status %s/%s: %w", bs.Namespace, bs.Name, err)
	}
	c.getLog().Infow("BreakglassSession status updated successfully", system.NamespacedFields(bs.Name, bs.Namespace)...)
	return nil
}
...
func (c SessionManager) DeleteBreakglassSession(ctx context.Context, bs *v1alpha1.BreakglassSession) error {
	if err := c.Delete(ctx, bs); err != nil {
		c.getLog().Errorw("Failed to delete BreakglassSession", append(system.NamespacedFields(bs.Name, bs.Namespace), "error", err.Error())...)
		return fmt.Errorf("failed to delete breakglass session: %w", err)
	}
	c.getLog().Infow("BreakglassSession deleted successfully", system.NamespacedFields(bs.Name, bs.Namespace)...)
