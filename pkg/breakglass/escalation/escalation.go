...
type EscalationFiltering struct {
	...
	FilterUserData   breakglass.ClusterUserGroup
...
func (ef EscalationFiltering) FilterForUserPossibleEscalations(ctx context.Context,
	...
	if cfg, errCfg := config.Load(); errCfg != nil {
		ef.Log.Warnw("Failed to load config for OIDC prefix normalization", "error", errCfg)
	} else if len(cfg.Kubernetes.OIDCPrefixes) > 0 {
		oidcPrefixes = cfg.Kubernetes.OIDCPrefixes
		originalGroups := make([]string, len(userGroups))
	...
	for _, esc := range escalations {
		...
		if len(oidcPrefixes) > 0 {
			normalizedAllowedGroups = breakglass.StripOIDCPrefixes(normalizedAllowedGroups, oidcPrefixes)
		}
