...
func (wc *BreakglassSessionController) sendSessionApprovalEmail(log *zap.SugaredLogger, session breakglassv1alpha1.BreakglassSession) bool {
	...
	if isScheduled {
		activationTime = session.Spec.ScheduledStartTime.UTC().Format("2006-01-02 15:04:05 UTC")
	}
	...
	params := mail.ApprovedMailParams{
		...
		ExpirationTime: session.Status.ExpiresAt.UTC().Format("2006-01-02 15:04:05 UTC"),
...
func (wc *BreakglassSessionController) sendSessionRejectionEmail(log *zap.SugaredLogger, session breakglassv1alpha1.BreakglassSession) {
	...
	params := mail.RejectedMailParams{
		...
		BrandingName:    brandingName,
		RejectedAt:      session.Status.RejectedAt.UTC().Format("2006-01-02 15:04:05 UTC"),
