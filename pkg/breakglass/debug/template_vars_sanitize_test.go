...
func TestTemplateVarsPreservedBeforeSerialization(t *testing.T) {
	for _, value := range []string{"registry/image:v1", "alice@example.com", "a,b", "worker-1\nhostNetwork: true", "---\nfoo", "a\u2028b"} {
		...
		require.Equal(t, value, pod["value"])
		require.Equal(t, value, auxiliary["value"])
		...
		require.Equal(t, value, (&DebugSessionController{}).buildVarsFromSession(session, spec)["value"])
		require.Equal(t, value, (&AuxiliaryResourceManager{}).buildVarsFromSession(session, spec)["value"])
	}
}
