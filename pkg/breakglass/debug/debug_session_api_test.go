...
func TestDebugSessionAPIController_ListSessions(t *testing.T) {
	...
	t.Run("filter sessions by state", func(t *testing.T) {
		...
		for _, s := range sessionList.Items {
			if s.Status.State == breakglassv1alpha1.DebugSessionStateActive {
...
func TestDebugSessionAPIController_JoinSession(t *testing.T) {
	...
	t.Run("cannot join non-active session", func(t *testing.T) {
		...
		pendingSession.Status.State = breakglassv1alpha1.DebugSessionStatePending
...
func TestDebugSessionAPIController_TerminateSession(t *testing.T) {
	...
	t.Run("terminate session as owner", func(t *testing.T) {
		...
		// Terminate
		fetchedSession.Status.State = breakglassv1alpha1.DebugSessionStateTerminated
...
func TestDebugSessionAPIController_ListTemplates(t *testing.T) {
	...
	templates := []breakglassv1alpha1.DebugSessionTemplate{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name: "standard-debug",
			},
	...
	t.Run("list all templates", func(t *testing.T) {
		...
		var templateList breakglassv1alpha1.DebugSessionTemplateList
	...
	t.Run("list pod templates", func(t *testing.T) {
		...
		var podTemplateList breakglassv1alpha1.DebugPodTemplateList
...
func TestDebugSessionAPIController_PermissionChecks(t *testing.T) {
	...
	t.Run("template with allowed groups", func(t *testing.T) {
		...
		var fetchedTemplate breakglassv1alpha1.DebugSessionTemplate
		err := fakeClient.Get(context.Background(), client.ObjectKey{
			Name: "restricted-debug",
		}, &fetchedTemplate)
		require.NoError(t, err)
...
func TestDebugSessionAPIController_JoinSessionErrors(t *testing.T) {
	...
	t.Run("join pending session", func(t *testing.T) {
		...
		assert.Equal(t, breakglassv1alpha1.DebugSessionStatePending, fetchedSession.Status.State)
	})
...
func TestDebugSessionAPIController_ApproveSessionErrors(t *testing.T) {
	...
	t.Run("approve active session (no approval required)", func(t *testing.T) {
		...
		assert.Equal(t, breakglassv1alpha1.DebugSessionStateActive, fetchedSession.Status.State)
...
func TestDebugSessionAPIController_HandleListDebugSessions(t *testing.T) {
	...
	t.Run("list requires authenticated user context", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&sessions[0], &sessions[1]).
			WithStatusSubresource(&sessions[0], &sessions[1]).
	...
	t.Run("filter by multiple states via repeated query params", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&sessions[0], &sessions[1]).
			WithStatusSubresource(&sessions[0], &sessions[1]).
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/debugSessions?state=Active&state=Pending", nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
		...
		require.NoError(t, err)
		assert.Equal(t, 2, response.Total)
		names := []string{response.Sessions[0].Name, response.Sessions[1].Name}
		assert.ElementsMatch(t, []string{"session-1", "session-2"}, names)
	})

	t.Run("filter by multiple states via comma-separated value", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&sessions[0], &sessions[1]).
			WithStatusSubresource(&sessions[0], &sessions[1]).
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/debugSessions?state=Active,Pending", nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
	...
	t.Run("filter by state case-insensitive", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&sessions[0], &sessions[1]).
			WithStatusSubresource(&sessions[0], &sessions[1]).
	...
	t.Run("filter by invalid state returns 400", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&sessions[0], &sessions[1]).
			WithStatusSubresource(&sessions[0], &sessions[1]).
	...
	t.Run("user filter does not leak unauthorized sessions", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/debugSessions?user=bob@example.com", nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
	...
	t.Run("isParticipant and activeParticipants computed correctly", func(t *testing.T) {
		...
		sessionWithParticipants := breakglassv1alpha1.DebugSession{
			ObjectMeta: metav1.ObjectMeta{
			...
			Spec: breakglassv1alpha1.DebugSessionSpec{
				...
				TemplateRef:         "standard-debug",
				RequestedBy:         "alice@example.com",
				InvitedParticipants: []string{"dave@example.com", "charlie@example.com"},
			},
			Status: breakglassv1alpha1.DebugSessionStatus{
				State:     breakglassv1alpha1.DebugSessionStateActive,
				StartsAt:  &now,
		...
		router2 := debugSessionAPITestRouter(t, ctrl, "dave@example.com", "", nil)
		...
		require.Equal(t, 1, response2.Total)
		...
		router3 := debugSessionAPITestRouter(t, ctrl, "charlie@example.com", "", nil)
...
func TestDebugSessionAPIController_HandleGetDebugSession(t *testing.T) {
	...
	t.Run("get existing session requires authenticated user context", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&session).
			WithStatusSubresource(&session).
	...
	t.Run("get existing session rejects unrelated user", func(t *testing.T) {
		...
		ctrl := NewDebugSessionAPIController(logger, fakeClient, nil, nil)
		router := debugSessionAPITestRouter(t, ctrl, "mallory@example.com", "", nil)
	...
	t.Run("get existing session allows active participant email", func(t *testing.T) {
		participantSession := session.DeepCopy()
		participantSession.Status.Participants = append(participantSession.Status.Participants, breakglassv1alpha1.DebugSessionParticipant{
			User:     "opaque-subject",
		...
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(participantSession).
			WithStatusSubresource(participantSession).
			Build()

		ctrl := NewDebugSessionAPIController(logger, fakeClient, nil, nil)
		router := debugSessionAPITestRouter(t, ctrl, "opaque-subject", "bob@example.com", nil)
	...
	t.Run("get existing session allows invited participant", func(t *testing.T) {
		invitedSession := session.DeepCopy()
		invitedSession.Spec.InvitedParticipants = []string{"dave@example.com"}

		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(invitedSession).
			WithStatusSubresource(invitedSession).
			Build()

		ctrl := NewDebugSessionAPIController(logger, fakeClient, nil, nil)
		router := debugSessionAPITestRouter(t, ctrl, "dave@example.com", "", nil)
	...
	t.Run("get existing session allows configured approver group", func(t *testing.T) {
	...
	t.Run("get existing session rejects participant after leaving", func(t *testing.T) {
		...
		leftSession.Spec.RequestedBy = "alice@example.com"
		leftSession.Status.Participants = append(leftSession.Status.Participants, breakglassv1alpha1.DebugSessionParticipant{
		...
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(leftSession).
			WithStatusSubresource(leftSession).
			Build()

		ctrl := NewDebugSessionAPIController(logger, fakeClient, nil, nil)
		router := debugSessionAPITestRouter(t, ctrl, "charlie@example.com", "", nil)
...
func TestDebugSessionAPIController_HandleListTemplates(t *testing.T) {
	...
	t.Run("list templates with allowed groups filter", func(t *testing.T) {
		...
		templateWithGroups.Spec.Allowed = &breakglassv1alpha1.DebugSessionAllowed{
	...
	t.Run("list templates resolves cluster patterns to actual cluster names", func(t *testing.T) {
		...
		wildcardTemplate := breakglassv1alpha1.DebugSessionTemplate{
		...
		prefixTemplate := breakglassv1alpha1.DebugSessionTemplate{
		...
		specificTemplate := breakglassv1alpha1.DebugSessionTemplate{
	...
	t.Run("list templates includes cluster availability info", func(t *testing.T) {
		...
		templateWithClusters := breakglassv1alpha1.DebugSessionTemplate{
		...
		templateWithoutClusters := breakglassv1alpha1.DebugSessionTemplate{
...
func TestDebugSessionAPIController_HandleListPodTemplates(t *testing.T) {
	...
	podTemplates := []breakglassv1alpha1.DebugPodTemplate{
		{
			ObjectMeta: metav1.ObjectMeta{
				Name: "ubuntu-debug",
			},
...
func TestDebugSessionAPIController_HandleCreateDebugSession(t *testing.T) {
	...
	templateWithClusterRestriction := breakglassv1alpha1.DebugSessionTemplate{
		ObjectMeta: metav1.ObjectMeta{
			Name: "restricted-debug",
		},
		Spec: breakglassv1alpha1.DebugSessionTemplateSpec{
	...
	t.Run("create session treats namespace as target namespace alias", func(t *testing.T) {
		...
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(templateWithUserNamespace, clusterConfig).
			Build()
		...
		body := `{"templateRef":"standard-debug","cluster":"production","namespace":"debug-pods","reason":"debugging issue"}`
		req := httptest.NewRequest(http.MethodPost, "/api/v1/debugSessions", strings.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
		...
		err = json.Unmarshal(w.Body.Bytes(), &response)
		require.NoError(t, err)
		assert.Equal(t, "breakglass-control", response.Namespace)
		assert.Equal(t, "debug-pods", response.Spec.TargetNamespace)
	})

	t.Run("create session rejects conflicting namespace aliases", func(t *testing.T) {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			Build()
		...
		body := `{"templateRef":"standard-debug","cluster":"production","namespace":"debug-a","targetNamespace":"debug-b"}`
		req := httptest.NewRequest(http.MethodPost, "/api/v1/debugSessions", strings.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
	...
	t.Run("rejects ambiguous ClusterConfig tenant aliases", func(t *testing.T) {
		firstTenantCluster := &breakglassv1alpha1.ClusterConfig{
			ObjectMeta: metav1.ObjectMeta{Name: "cluster-config-prod-a", Namespace: "tenant-a"},
			Spec: breakglassv1alpha1.ClusterConfigSpec{
				Tenant: "production",
			},
			Status: breakglassv1alpha1.ClusterConfigStatus{
		...
		secondTenantCluster := &breakglassv1alpha1.ClusterConfig{
			ObjectMeta: metav1.ObjectMeta{Name: "cluster-config-prod-b", Namespace: "tenant-b"},
			Spec: breakglassv1alpha1.ClusterConfigSpec{
				Tenant: "production",
			},
			Status: breakglassv1alpha1.ClusterConfigStatus{
		...
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&template, firstTenantCluster, secondTenantCluster).
			Build()
	...
	t.Run("create session returns warnings when scheduling option is defaulted", func(t *testing.T) {
		templateWithScheduling := breakglassv1alpha1.DebugSessionTemplate{
			ObjectMeta: metav1.ObjectMeta{
...
func TestDebugSessionAPIController_HandleJoinDebugSession(t *testing.T) {
	...
	t.Run("join session successfully", func(t *testing.T) {
		...
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
			WithObjects(&session, &breakglassv1alpha1.ClusterConfig{ObjectMeta: metav1.ObjectMeta{Name: "production", Namespace: "default"}}).
			WithStatusSubresource(&session).
			Build()
...
func TestDebugSessionAPIController_HandleLeaveDebugSession(t *testing.T) {
	...
	t.Run("leave expired session hides lifecycle details from non participant", func(t *testing.T) {
		...
		assert.Contains(t, w.Body.String(), "user is not a participant in this session")
		assert.NotContains(t, w.Body.String(), "expired")
	})

	t.Run("leave non-active session uses lifecycle error phrasing", func(t *testing.T) {
		session := breakglassv1alpha1.DebugSession{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-session",
				Namespace: "default",
		...
		assert.Equal(t, http.StatusBadRequest, w.Code)
		assert.Contains(t, w.Body.String(), "cannot leave session in state 'Pending'")
	})
...
func TestDebugSessionAPIController_HandleRenewDebugSession(t *testing.T) {
	...
	t.Run("renew rejects extension beyond max duration", func(t *testing.T) {
		...
		assert.Equal(t, int32(0), updatedSession.Status.RenewalCount)
		require.NotNil(t, updatedSession.Status.ExpiresAt)
		assert.WithinDuration(t, expiresNearLimit.Time, updatedSession.Status.ExpiresAt.Time, time.Second)
	})

	t.Run("renew requires start time for max duration enforcement", func(t *testing.T) {
		session := breakglassv1alpha1.DebugSession{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-session",
				Namespace: "default",
...
func TestConvertSelectorTerms(t *testing.T) {
	tests := []struct {
		name     string
		terms    []breakglassv1alpha1.NamespaceSelectorTerm
			...
			terms: []breakglassv1alpha1.NamespaceSelectorTerm{
				{
					MatchExpressions: []breakglassv1alpha1.NamespaceSelectorRequirement{
						...
						{
							Key:      "restricted",
							Operator: breakglassv1alpha1.NamespaceSelectorOpDoesNotExist,
						},
					},
				},
			},
			...
			terms: []breakglassv1alpha1.NamespaceSelectorTerm{
				...
				},
				{
					MatchExpressions: []breakglassv1alpha1.NamespaceSelectorRequirement{
						{
							Key:      "env",
							Operator: breakglassv1alpha1.NamespaceSelectorOpExists,
						},
					},
				},
			},
			...
			existingSessions: []breakglassv1alpha1.DebugSession{
				{
					...
					Status: breakglassv1alpha1.DebugSessionStatus{State: breakglassv1alpha1.DebugSessionStateFailed},
				},
			},
			userEmail:   "user2@example.com",
			expectError: false,
	...
	tests := []struct {
			...
			bindings: []breakglassv1alpha1.DebugSessionClusterBinding{
				{
					...
					Spec: breakglassv1alpha1.DebugSessionClusterBindingSpec{
						TemplateRef: &breakglassv1alpha1.TemplateReference{Name: "template2"},
			...
			bindings: []breakglassv1alpha1.DebugSessionClusterBinding{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "binding1", Namespace: "default"},
					Spec: breakglassv1alpha1.DebugSessionClusterBindingSpec{
						TemplateRef: &breakglassv1alpha1.TemplateReference{Name: "other-template"}, // Wrong template!
						Clusters:    []string{"test-cluster"},
					},
				},
			},
		...
		},
		{
			...
			template: &breakglassv1alpha1.DebugSessionTemplate{
				...
				Spec:       breakglassv1alpha1.DebugSessionTemplateSpec{}, // No Allowed field
			},
...
func TestDebugSessionAPIController_CreateWithExtraDeployValues(t *testing.T) {
	...
	templateWithVariables := breakglassv1alpha1.DebugSessionTemplate{
		ObjectMeta: metav1.ObjectMeta{
...
func TestHandleListDebugSessions_StateValidation(t *testing.T) {
	...
	buildRouter := func(username string) *gin.Engine {
		fakeClient := fake.NewClientBuilder().
			WithScheme(scheme).
		...
		ctrl := NewDebugSessionAPIController(logger, fakeClient, nil, nil)
		if username != "" {
			return debugSessionAPITestRouter(t, ctrl, username, "", nil)
		}
		router := gin.New()
		rg := router.Group("/api/v1/" + ctrl.BasePath())
