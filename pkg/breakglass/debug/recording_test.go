...
func TestInjectTerminalRecordingContract(t *testing.T) {
	...
	if err := injectTerminalRecording(spec, ds, template, "registry.example/recorder@sha256:abc"); err != nil {
	...
	if !hasEnv(spec.Containers[0].Env, TerminalRecordingRedactionEnv) {
		t.Fatal("expected recording marker on workload container")
	}
	if got := recordingCorrelationID(ds.Namespace, ds.Name); got != recordingCorrelationID(ds.Namespace, ds.Name) {
		t.Fatalf("correlation id is not stable: %q", got)
	}
}
