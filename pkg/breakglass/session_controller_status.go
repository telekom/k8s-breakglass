...
func (wc *BreakglassSessionController) setSessionStatus(c *gin.Context, sesCondition breakglassv1alpha1.BreakglassSessionConditionType) {
	...
	case breakglassv1alpha1.SessionConditionTypeApproved:
		...
		if emailSent {
			c.Header("X-Notification-Sent", "true")
		}
...
func (wc *BreakglassSessionController) getActiveBreakglassSession(ctx context.Context,
	...
	wc.log.Debugw("Querying for active breakglass session", "user", username, "cluster", clustername, "group", system.RedactGroupName(group))
	...
	if len(validSessions) == 0 {
		wc.log.Infow("No active breakglass session found", "user", username, "cluster", clustername, "group", system.RedactGroupName(group))
		...
		wc.log.Error("There is more than a single active breakglass session; this should not happen",
			...
			zap.String("group", system.RedactGroupName(group)))
	}
...
func (wc *BreakglassSessionController) handleGetBreakglassSessionStatus(c *gin.Context) {
	...
	// Use request context with timeout to ensure we abort early if the client disconnects.
	// Timeout is configurable via APIContextTimeout.
