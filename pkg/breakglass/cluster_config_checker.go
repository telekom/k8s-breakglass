...
func (ccc ClusterConfigChecker) validateOIDCFromIdentityProvider(ctx context.Context, cc *telekomv1alpha1.ClusterConfig, lg *zap.SugaredLogger) (*rest.Config, error) {
	...
	// Validate cluster CA secret if configured
	if ref.CASecretRef != nil {
		...
		if _, ok := caSec.Data[caKeyName]; !ok {
			msg := "Referenced cluster CA secret missing key: " + caKeyName
			lg.Warnw(msg,
				"cluster", cc.Name,
				"secret", ref.CASecretRef.Name,
				"secretNamespace", ref.CASecretRef.Namespace)
...
func (ccc ClusterConfigChecker) setStatusAndEvent(ctx context.Context, cc *telekomv1alpha1.ClusterConfig, phase, message, eventType string, lg *zap.SugaredLogger) error {
	...
	// persist status: use Status().Update() as primary since ClusterConfig has status subresource
	// (regular Update() ignores status field when subresource is enabled)
	if err := ccc.Client.Status().Update(ctx, cc); err != nil {
		lg.Warnw("ClusterConfig Status().Update failed; attempting full Update (for fake client compatibility)", "cluster", cc.Name, "error", err)
		if err2 := ccc.Client.Update(ctx, cc); err2 != nil {
			lg.Warnw("failed to update ClusterConfig via full Update", "cluster", cc.Name, "error", err2)
			// return the underlying update error to caller
			return err2
		}
		lg.Debugw("ClusterConfig updated via full Update", "cluster", cc.Name)
	} else {
