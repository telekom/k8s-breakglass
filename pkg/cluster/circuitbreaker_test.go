...
func TestIsTransientError(t *testing.T) {
	tests := []struct {
		...
		{"dial tcp with connection refused", fmt.Errorf("dial tcp 10.0.0.1:6443: connect: connection refused"), true},
		{"dial timeout", fmt.Errorf("dial timeout"), true},
		{"dial tcp no such host", fmt.Errorf("api error: dial tcp: lookup my-cluster on 10.96.0.10:53: no such host"), false},
