...
func TestClusterConfigCleanupUsesRetainedAndUnknownInventory(t *testing.T) {
	for _, evidence := range []string{"none", "operation", "unknown kubectl operation", "created only", "name only", "empty child", "pod name only", "blank pod", "uid only", "kind only", "child uid only"} {
		t.Run(evidence, func(t *testing.T) {
			...
			case "unknown kubectl operation":
				session.Status.KubectlDebugStatus = &breakglassv1alpha1.KubectlDebugStatus{
					Operations: []breakglassv1alpha1.KubectlDebugOperation{{ID: "op", State: breakglassv1alpha1.KubectlDebugOperationUnknown}},
				}
			case "created only":
