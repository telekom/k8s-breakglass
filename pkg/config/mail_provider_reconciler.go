...
func (r *MailProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !validationResult.IsValid() {
		...
		if statusErr := r.applyStatus(ctx, &mp); statusErr != nil {
...
func (r *MailProviderReconciler) applyStatus(ctx context.Context, mp *breakglassv1alpha1.MailProvider) error {
	return ssa.ApplyMailProviderStatus(ctx, r.Client, mp)
}
