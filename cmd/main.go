	...
	go func() {
		...
		if certRotatorEnabled == "" || certRotatorEnabled == "true" {
			...
			if _, err := cert.SetupRotator(mgr, "breakglass-webhook", false, setupFinished); err != nil {
				...
				log.Infow("Certificate setup initiated, waiting for cert-rotator to signal readiness")
				...
				for !certReady {
					select {
					case <-setupFinished:
						log.Infow("✓ Webhook certificates ready and loaded", "secondsWaited", secondsWaited)
						certReady = true
					case <-ticker.C:
						secondsWaited += 10
						log.Infow("Still waiting for webhook certificates to be generated by cert-rotator", "secondsWaited", secondsWaited, "deadline", "60s", "namespace", podNamespace, "secretName", webhookSecretName)
						log.Debugw("Certificate readiness check", "setupFinished_channel_closed", false, "elapsed_seconds", secondsWaited)
					case <-timeout:
						log.Fatalf("✗ Timeout waiting for webhook certificates after 60s - certificate generation failed or stalled. Check: 1) webhook-certs secret exists in %s namespace with data, 2) cert-rotator pod logs for errors, 3) RBAC permissions for secret access", podNamespace)
					}
				}
				// Register webhooks immediately after certificates are ready
				if enableWebhookMgr == "" || enableWebhookMgr == "true" {
					log.Infow("Certificates ready, proceeding with webhook registration", "webhooksToRegister", []string{"BreakglassSession", "BreakglassEscalation", "ClusterConfig", "IdentityProvider"})
