...
- **Idle timeout design & session activity tracking** (#8, #312, #314): Added design document for idle timeout and last-used fields (`docs/design/idle-timeout.md`). Implemented full idle timeout and activity tracking feature:
  ...
  - **Metrics**: `breakglass_session_activity_requests_total` (labels: `cluster`, `granted_group`), `breakglass_session_activity_flushes_total`, `breakglass_session_activity_flush_errors_total`, `breakglass_session_idle_expired_total` (label: `cluster`). Fixed activity request metric cardinality by replacing unbounded `session` label with bounded `granted_group`.
