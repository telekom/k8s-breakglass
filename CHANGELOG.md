...
- **BreakglassSession approver group cache isolation**: Approval authorization now caches approver group membership per target cluster and user, and target-cluster group lookup results take precedence over generic request-context groups. This prevents group membership resolved for one cluster from authorizing approval checks for another cluster during the same request context.
