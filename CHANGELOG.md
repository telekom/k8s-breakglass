...
- **ClusterConfig readiness enforced for escalation sessions**: The Escalation API now hides active escalations for registered clusters whose `ClusterConfig` has `Ready=False`, and normal breakglass session creation rejects requests for those unready clusters instead of relying only on the escalation object's own `Ready` condition.
