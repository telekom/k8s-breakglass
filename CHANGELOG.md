...
- **Debug log redaction for sensitive data (F8)**: Log statements at all levels no longer emit raw OIDC group name lists, session approval tokens, or JWT token group lists. Where group lists previously appeared, only counts are logged (`groupCount`, `userGroupCount`, `approverGroupCount`, `finalGroupCount`, etc.) — raw group-name arrays and member email/username lists are removed. Affected files: `group_checker.go`, `escalation.go`, `escalation_manager.go`, `escalation_controller.go`, `escalation_status_updater.go`, `session_controller_actions.go`, `session_controller_approval_utils.go`, and `debug_session_api.go`. Approval tokens are logged only as their byte length (`tokenLen`), and JWT `rawTokenGroups` fields are omitted — only the count is logged. Per-group and per-escalation name context keys (e.g. `"group"`, `"escalation"`) are retained for operational traceability. This prevents bulk identity data from appearing in log aggregation systems at any log level when verbose logging is enabled.
