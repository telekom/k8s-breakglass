...
- **DebugSession cluster binding ClusterConfig lookup**: `DebugSessionClusterBinding.spec.clusters`, DebugSession creation, DebugSession template cluster listings, and `GET /api/clusterBindings/forCluster/{cluster}` now resolve `ClusterConfig` resources by name across namespaces, while duplicate names remain ambiguous and fail closed with `409 Conflict` on direct cluster-binding lookups.
