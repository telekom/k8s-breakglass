...
- **CEL validation expressions on CRDs** ([#313](https://github.com/telekom/k8s-breakglass/issues/313)): Added `x-kubernetes-validations` CEL rules to `BreakglassEscalationSpec` (approver required, blockSelfApproval guards, IDP field exclusivity), `SessionLimitsOverride` (unlimited/limits exclusivity), and `DenyPolicySpec` (at least one rule required). CRD manifests regenerated.
