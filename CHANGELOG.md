...
- **CLI usability improvements (part 1)** ([#544](https://github.com/telekom/k8s-breakglass/issues/544)): Unknown `-o/--output` format errors now include supported choices in `session`, `debug`, and `escalation` commands; `watch --interval` help text now documents Go duration syntax (`2s`, `1m`); debug `--state` help now documents comma-separated filters; `bgctl config delete-context` now suggests `bgctl config get-contexts` when a context is missing.
