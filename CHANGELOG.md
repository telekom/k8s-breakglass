...
- **JWT audience claim validation (SEC-005)** ([#472](https://github.com/telekom/k8s-breakglass/issues/472)): New `IdentityProvider.spec.oidc.expectedAudience` field enables opt-in JWT `aud` claim validation, preventing cross-service token confusion from other OIDC clients at the same provider. Requires a matching audience protocol mapper in the IDP. When empty (default), audience validation is skipped for backwards compatibility
