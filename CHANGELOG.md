...
- **CLI usability improvements (part 1)** ([#544](https://github.com/telekom/k8s-breakglass/issues/544)): Unknown `-o/--output` format errors now include supported choices across `session`, `debug`, and `escalation` commands (including direct object subcommands); root `--output` help text now documents `wide` and command-dependent support; `watch --interval` help text now documents Go duration syntax (`2s`, `1m`); debug `--state` flag now supports multiple values via comma-separated input (e.g. `active,pending`) — the server accepts repeated `state=` query params and comma-separated values; `bgctl config delete-context` now suggests `bgctl config get-contexts` when a context is missing.
