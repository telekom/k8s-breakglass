...
- **DebugSession cluster binding ClusterConfig lookup**: `DebugSessionClusterBinding.spec.clusters` and `GET /api/clusterBindings/forCluster/{cluster}` now resolve `ClusterConfig` resources by name across namespaces, while duplicate names remain ambiguous and fail closed.
