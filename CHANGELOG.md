...
- **Idle timeout design & session activity tracking** (#8, #312, #314): Added design document for idle timeout and last-used fields (`docs/design/idle-timeout.md`). Implemented session activity tracking infrastructure: added `lastActivity` and `activityCount` fields to `BreakglassSessionStatus`, authorization webhook records activity via a buffered `ActivityTracker` (30s flush interval) to avoid API server overhead. New Prometheus metrics: `breakglass_session_activity_requests_total`, `breakglass_session_activity_flushes_total`, `breakglass_session_activity_flush_errors_total`. This lays the groundwork for idle timeout session expiration (#312).
