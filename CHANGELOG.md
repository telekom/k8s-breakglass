...
- **Release: keep prerelease images off `latest`**: Release image assembly now publishes the `latest` image tag only for stable `vX.Y.Z` tags. Prerelease tags such as `v1.2.3-rc.1` keep their explicit version tag without moving `latest`.
