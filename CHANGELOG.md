...
- **OIDC proxy redirect SSRF**: The unauthenticated OIDC proxy no longer follows upstream `30x` responses. Only the first hop was validated against the configured-authority allowlist, so a trusted-but-redirecting identity provider could steer a server-side request to an arbitrary host (CWE-918). The `30x` status is still relayed with `Location`/`Set-Cookie` stripped. Redirect following can be restored for a non-conforming IdP via the new optional `server.allowOIDCProxyRedirects` setting, which defaults to the secure behaviour when unset.
- **OIDC proxy response size**: Proxied upstream responses are now capped at 1 MiB instead of being copied unbounded, and a truncated response is logged and counted as `response_too_large` rather than reported as success (a body of exactly 1 MiB is relayed in full and still counts as a success). Upstream responses with an empty body (notably a refused redirect) now relay their real status instead of `200`.
- **OIDC proxy plaintext authority**: A configured `http://` OIDC authority — reachable only via legacy file-based config, since the `IdentityProvider` CRD pins `^https://.+` — now logs a loud `oidc_proxy_insecure_http_authority` warning explaining that the outbound first hop is unauthenticated and spoofable. Startup is unaffected.
- **Approval authorization fail-open**: A failed cluster-side approver group lookup no longer silently substitutes unverified JWT-claim groups into the verified-groups check. The failure is now treated as "no verified groups" and falls through to the pre-existing, explicitly scoped request-context fallback, preserving today's outcomes without introducing lockouts. The underlying error is always logged at `Error` level, and a load-bearing unverified allow emits a `session.approval_unverified_groups` audit event plus the new `breakglass_approval_group_lookup_failures_total` and `breakglass_approval_unverified_group_decisions_total` metrics.
...
- **bgctl local secret file permissions**: `bgctl` now tightens existing config and file-backed token cache files to owner-only permissions (`0600`) before writing inline OIDC client secrets or cached tokens, preventing permissive pre-created files from remaining group/world-readable. Existing writable files in non-writable directories continue to be updated through a secure in-place fallback.
- **Production RBAC least privilege**: The production `config/base` render no longer includes the broad auth-checker impersonation helper that granted the manager ServiceAccount unrestricted `users` and `groups` impersonation. Debug-session impersonation remains an explicit opt-in grant for named ServiceAccounts.
...
- **Audit sink circuit breaker implementation**: The audit Kafka/webhook sink circuit breaker now uses the maintained `github.com/sony/gobreaker/v2` state machine instead of a custom in-house implementation while preserving the existing sink metrics, state reporting, force-open/close controls, and half-open probe limits.
...
- **Debug session kubectl-debug status updates preserve live state**: Ephemeral container injection, pod copy, node debug pod creation, and kubectl-debug cleanup now merge their status changes into the latest `DebugSession` status, preserving concurrent renewals, participant changes, and other live lifecycle fields.
...
- **bgctl usage errors now exit consistently**: Nested unknown subcommands and argument/flag usage errors now return exit code `2` instead of printing parent help with success or collapsing into the generic error code.
- **ClusterConfig readiness enforced for escalation sessions**: The Escalation API now hides active escalations for registered clusters whose `ClusterConfig` has `Ready=False` or an ambiguous duplicate name, and normal breakglass session creation rejects requests for those unready or ambiguous clusters instead of relying only on the escalation object's own `Ready` condition.
...
- **DebugSession binding session limits**: `maxActiveSessionsPerUser` now keys limits by requester username when an email claim is absent and ignores active sessions whose `status.expiresAt` is already in the past, preventing unrelated no-email users or stale sessions from consuming binding capacity.
...
- **UI screenshot workflow failures are now visible**: The UI screenshot GitHub Actions workflow still uploads the screenshot gallery and Playwright report when screenshot generation fails, but now marks the job failed after diagnostics are available instead of allowing a green run with stale or missing screenshots.
- **Frontend: block invalid debug session create submissions**: The debug session create flow now keeps submission disabled while namespace constraints or extra deploy variable validation errors are present, preventing invalid `targetNamespace` or `extraDeployValues` payloads from being sent after inline validation marks the fields invalid.
- **Frontend: hide authenticated mobile navigation before login**: The mobile header now hides the hamburger navigation while signed out and gives the signed-in profile menu a 44px touch target on mobile and tablet viewports.
- **Release: classify stable and prerelease tags consistently**: Release image assembly now publishes the `latest` image tag only for stable `vX.Y.Z` tags, GitHub Releases for prerelease tags are marked as prereleases and cannot become the repository's Latest release, and `+build` metadata tags are rejected before Docker tag publication.
...
- **CLI usability improvements (part 1)** ([#544](https://github.com/telekom/k8s-breakglass/issues/544)): Unsupported `-o/--output` format errors now include supported choices across `session`, `debug`, and `escalation` commands (including direct object subcommands), using the wording "unsupported output format" to distinguish known-but-unsupported formats from truly unknown values; root `--output` help text now documents `wide` and command-dependent support; `watch --interval` help text now documents Go duration syntax (`2s`, `1m`); debug `--state` flag now accepts comma-separated state values (e.g. `Active,Pending`) — the client splits on commas and sends repeated `state=` query params which the server filters case-insensitively; `bgctl config delete-context` now suggests `bgctl config get-contexts` when a context is missing.; `debug watch --show-full` now respects the `-o/--output` flag (previously always output JSON regardless of the requested format); the debug sessions list API endpoint now returns HTTP 400 for unrecognized `state=` filter values instead of silently ignoring them.
- **Frontend: UI/UX Refactoring for Theme and Contrast Toggles**: Relocated theme and high-contrast toggles to a dedicated "utilities" area in the application header for better visibility and accessibility. Added a standard icon for High Contrast mode (eye icon) and improved toggle interaction states with premium animations. Integrated theme and contrast controls into the mobile flyout menu to ensure full functionality across all device sizes. Refined layout spacing and added visual dividers for a more professional aesthetic.

### Added

- **Frontend: comprehensive accessibility improvements (WCAG 2.1 AA + AAA)**: Fixed duplicate `<main>` landmarks across all views, added ARIA roles and labels to interactive components (timers, loading spinners, action buttons, navigation), implemented route-change focus management, replaced opacity-based muted text with explicit color tokens for WCAG AA contrast compliance, added a user-facing High Contrast mode toggle (persisted to localStorage) with dedicated CSS custom property overrides achieving WCAG AAA 7:1 contrast ratios for both light and dark themes, enforced WCAG AAA 44×44px minimum touch/click target sizes on interactive elements in High Contrast mode (`[data-high-contrast="true"]`), expanded `@media (forced-colors: active)` support for Windows High Contrast Mode, added high-contrast mode to CI screenshot tests alongside light and dark themes, and added automated accessibility regression testing in CI (`eslint-plugin-vuejs-accessibility` for lint-time checks, `@axe-core/playwright` E2E tests validating 6 pages + 3 error pages + 4 modals across 4 theme modes against WCAG 2.1 AA + AAA rules). Primary buttons retain Telekom Magenta (`#e20074`, AA-level 4.68:1 contrast) per brand guidelines; all other interactive elements meet AAA contrast. Scale shadow-DOM issues outside our control are suppressed via rule disabling and node filtering. See `frontend/SCALE_DEVIATIONS.md` for full contrast deviation rationale (PR #616).

- **Frontend: UX improvements**: Replaced heavy `<scale-divider>` section separators in cards with lighter CSS-only lines, replaced redundant static eyebrow text with contextual info (cluster name on escalation cards, cluster label on session cards), added missing `--space-2xs` design token used across 9 files, toned down the background gradient for improved readability, refactored Session Review page to use shared `PageHeader`/`LoadingState`/`EmptyState` components for consistency, removed duplicate deadline info from My Pending Requests footer (already displayed in the meta grid with countdown timers), and increased the header app-name font size to meet WCAG AAA contrast-enhanced large text thresholds while preserving the Telekom Magenta brand color.

- **Frontend: dark mode contrast and spacing fixes**: Fixed poor text-on-background contrast for all four semantic tone chips in dark mode (info, success, warning, danger) by replacing low-contrast text colors with AAA-compliant values in `base.css`. Replaced Telekom Magenta accent color on status details, session subtitles, and countdown timers with accessible text tokens that meet WCAG AAA on dark surfaces. Widened button/action gaps from 4 px to 8 px in `BreakglassCard`, `SessionBrowser`, and `PendingApprovalsView` to eliminate touch-target collisions. Increased approval card shell padding from 2 px to 8 px. Added explicit tone text colors to `ApprovalModalContent` info and warning blocks. Replaced all hardcoded `rem`/`rgba()` CSS values in `SessionApprovalView` with design tokens. Added responsive mobile fallback for the session timeline grid to prevent narrow-width column collisions.
...
- **Debug log redaction for sensitive data (F8)**: Log statements at all levels no longer emit raw OIDC group name arrays, member email/username lists, session approval tokens, or JWT token group lists. Where group lists previously appeared, only counts are logged (`groupCount`, `userGroupCount`, `approverGroupCount`, `finalGroupCount`, etc.) — raw group-name arrays and member email/username lists are removed. Approval tokens are logged only as their byte length (`tokenLen`), and JWT `rawTokenGroups` fields are omitted — only the count is logged. Per-group and per-escalation name context keys (e.g. `"group"`, `"escalation"`) are retained for operational traceability. Group name hints in log fields use a partial mask (first 3 chars + `***` for longer names; fully masked for ≤4 chars) — access controls on log aggregation systems remain necessary for full protection. Affected files span the entire session, escalation, webhook, mail queue, and debug subsystems: `group_checker.go`, `escalation.go`, `escalation_manager.go`, `escalation_controller.go`, `escalation_status_updater.go`, `session_controller_actions.go`, `session_controller_approval_utils.go`, `debug_session_api.go`, `webhook/`, and `logging.go`.
...
- **CleanupRoutine: AuditManager now wired from cmd/main.go**: `CleanupRoutine.AuditManager` was always `nil` because `startBackgroundRoutines` never set it; audit events for debug session expiry and approval timeouts were silently dropped. Fixed by exposing `audit.Service.Manager()` and passing the result into the struct literal.
- **IDP restriction enforcement gap for per-role IDP fields**: Session validation (`validateSessionIdentityProviderAuthorization`) now considers `allowedIdentityProvidersForRequests` when the legacy `allowedIdentityProviders` field is empty, preventing unrestricted access when only per-role IDP fields are configured. Similarly, `AllowIDPMismatch` computation in session creation now accounts for both fields.
- **Frontend: redact bearer token from dev debug logs**: Dev token logging in `httpClient.ts` now logs token metadata (length) instead of the full `Authorization: Bearer <token>` value, preventing accidental credential exposure in browser console during development
- **Audit: sensitive events are no longer sampled; manager-queue overflow now falls back to sync write** — `shouldSample()` now exempts all sensitive event types from sampling regardless of `SampleRate` configuration. Sensitive types include: session request/approve/deny/reject/expire/revoke/withdraw/drop, access denial, secret CRUD, auth failure, debug session create/start/terminate/fail/expire/approval-timeout, cluster role binding create/delete, resource impersonation, policy bypass/violation, and pod security deny/warning/override. When the manager's main async queue is full, sensitive events fall back to a **direct synchronous write** path, blocking up to `WriteTimeout` to reduce drops from manager-queue overflow. This fallback does not intercept per-sink queue drops (e.g. `queue_full` or `circuit_open`). On success, `breakglass_audit_events_processed_total` and `breakglass_audit_sensitive_events_sync_written_total` are both incremented; on failure, `breakglass_audit_sink_errors_total` is incremented with `error_type=sensitive_sync_fallback`.
...
- **Frontend: guard missing route params in DebugSessionDetails** ([#536](https://github.com/telekom/k8s-breakglass/issues/536)): Added type guard for `route.params.name` and early return with clear error message when the session name is missing from the URL
- **Frontend: typed defineEmits in BreakglassSessionCard** ([#545](https://github.com/telekom/k8s-breakglass/issues/545)): Converted string-array `defineEmits` to typed object syntax for compile-time payload type safety
- **WaitingForScheduledTime sessions stuck forever** ([#543](https://github.com/telekom/k8s-breakglass/issues/543)): Sessions in `WaitingForScheduledTime` state with nil or zero `ScheduledStartTime` are now expired instead of silently skipped, preventing indefinite resource retention
- **Debug session approval timeout only in cleanup task** ([#547](https://github.com/telekom/k8s-breakglass/issues/547)): `handlePendingApproval()` in the debug session reconciler now checks `DEBUG_SESSION_APPROVAL_TIMEOUT` directly, failing timed-out sessions immediately rather than waiting for the periodic cleanup routine
- **Approval email failure silent** ([#546](https://github.com/telekom/k8s-breakglass/issues/546)): Session approval API response now includes `X-Notification-Sent: true` header when the approval email is successfully enqueued, giving approvers visibility into notification delivery status
- **Helm: fail fast on missing kubeconfigSecretRef** ([#532](https://github.com/telekom/k8s-breakglass/issues/532)): Added `required` validation for `cluster.kubeconfigSecretRef.name` and `cluster.kubeconfigSecretRef.namespace` in the ClusterConfig Helm template so chart rendering fails early with a clear error instead of producing an invalid manifest
- **Helm: document ConfigMap integration** ([#533](https://github.com/telekom/k8s-breakglass/issues/533)): Added a note in the chart README clarifying that circuit breaker and telemetry ConfigMaps are not automatically mounted and must be manually integrated into the controller Deployment
...
- **CEL validation expressions on CRDs** ([#313](https://github.com/telekom/k8s-breakglass/issues/313)): Added `x-kubernetes-validations` CEL rules to `BreakglassEscalationSpec` (approver required, blockSelfApproval guards, IDP field exclusivity), `SessionLimitsOverride` (unlimited/limits exclusivity), and `DenyPolicySpec` (at least one rule required). Go webhook validation mirrors all CEL rules as defense-in-depth. Added `MaxItems` constraints to bound CEL cost estimation (approvers.groups=30, users=100, hiddenFromUI=30, IDP lists=20). CRD manifests regenerated. Added CRD schema validation test (offline + envtest API server) and `make validate-crds` target. CI workflow split: `lint-test` → parallel `lint` + `unit-tests` + `crd-validation` + `npm-audit` jobs.
  - **Breaking**: `DenyPolicy` resources with zero `rules` and no `podSecurityRules` are now rejected on create/update. Existing empty `DenyPolicy` resources must be updated to include at least one rule before upgrading.
  - **Breaking**: `MaxItems` constraints added to array fields — `approvers.users` (100), `approvers.groups` (30), `approvers.hiddenFromUI` (30), `allowedIdentityProviders*` (20 each). Existing resources exceeding these limits will be rejected on update.
- **OpenTelemetry tracing support** ([#317](https://github.com/telekom/k8s-breakglass/issues/317)): Added distributed tracing via OpenTelemetry. Configurable via CLI flags (`--otel-enabled`, `--otel-exporter`, `--otel-endpoint`), environment variables (`OTEL_ENABLED`, `OTEL_EXPORTER_OTLP_ENDPOINT`), or `config.yaml` (`telemetry` section). Supports OTLP gRPC, stdout, and no-op exporters. Gin middleware creates spans for every HTTP request. Graceful shutdown flushes pending spans. See `docs/opentelemetry-tracing.md`.
- **Gateway API support for webhook exposure** ([#316](https://github.com/telekom/k8s-breakglass/issues/316)): Added a kustomize Component at `config/components/gateway-api/` that replaces the default Ingress with a Gateway API HTTPRoute and ReferenceGrant. Includes implementation examples for Istio, Envoy Gateway, and Kong, with comprehensive documentation at `docs/gateway-api-configuration.md`. The component is opt-in and does not affect existing Ingress-based deployments.
- **Circuit breaker for spoke cluster communication** ([#419](https://github.com/telekom/k8s-breakglass/issues/419)): Added per-cluster circuit breaker in `pkg/cluster/` that detects unreachable spoke clusters and fails fast instead of blocking on TCP timeout. Configurable thresholds (failure/success count, open duration) via `config.yaml`. Emits Prometheus metrics for circuit state, transitions, rejections, and consecutive failures per cluster. Transient error classification ensures auth/config errors don't trip the breaker.
...
- **Copy-to-clipboard for kubectl exec command** ([#398](https://github.com/telekom/k8s-breakglass/issues/398)): Added a "Copy" button next to the `kubectl exec` command on the Debug Session Details page, with visual "Copied!" feedback. Introduced `useClipboard` composable with Clipboard API support and textarea fallback.
...
- **Breaking — Webhook OIDC prefix matching** ([#539](https://github.com/telekom/k8s-breakglass/issues/539)): Replaced O(n*m*k) triple-nested loop in `authorizeViaSessions` with O(n) map-based lookup. This changes matching semantics from `HasPrefix+HasSuffix` to exact `prefix+group` equality, which is stricter but consistent with `StripOIDCPrefixes` elsewhere. Groups previously matched via suffix (e.g., `oidc:cluster-admins` matching allowed group `admins`) will no longer match. **Migration**: verify that all groups in your BreakglassEscalation `allowedGroups` exactly match the OIDC group name without prefix; if your configuration relied on suffix matching, update the allowed group entries to include the full group name
...
- **Route-level code splitting for frontend views** ([#422](https://github.com/telekom/k8s-breakglass/issues/422)): Converted 10 view imports from static to dynamic `() => import(...)` in the Vue Router, reducing the main bundle from 492.92 kB to 331.21 kB (33% reduction). `BreakglassView` kept eager as the home page.
- **Decomposed DebugSessionCreate.vue into sub-components** ([#422](https://github.com/telekom/k8s-breakglass/issues/422)): Extracted `ClusterSelectGrid`, `BindingOptionsGrid`, and `SessionConfigForm` from the 1,987-line monolithic SFC, reducing it to 858 lines. Each sub-component owns its template, local state, and styles.
- **Replaced frontend README boilerplate** ([#422](https://github.com/telekom/k8s-breakglass/issues/422)): Replaced Vue/Vite scaffolding template text with project-specific header describing the tech stack and prerequisites.
...
- **Multi-arch release builds on native runners**:
  ...
  - ARM builds run on the GitHub-hosted `ubuntu-24.04-arm` runner
  - Artifactory push preserves the full multi-arch manifest instead of re-tagging a single-platform pull
...
- **Debug Session Details Polling Lifecycle**: `DebugSessionDetails` now re-evaluates polling on session state transitions so polling stops when sessions leave active/pending states
