...
- **BreakglassSession approver group cache isolation**: Approval authorization now caches approver group membership per target cluster and user, preventing group membership resolved for one cluster from authorizing approval checks for another cluster during the same request context.
