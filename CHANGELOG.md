...
- **Auth and bgctl hardening follow-ups** ([#472](https://github.com/telekom/k8s-breakglass/issues/472)): Multi-IDP auth now returns accurate client messages for unknown-issuer vs transient/rate-limited JWKS failures. `bgctl update` now uses command-scoped request contexts with an explicit HTTP timeout, removes partially extracted binaries on extraction errors, and propagates non-EOF probe-read errors during size-limited archive extraction
