...
- **Auth and bgctl hardening follow-ups** ([#472](https://github.com/telekom/k8s-breakglass/issues/472)): Multi-IDP auth now returns accurate client messages for unknown-issuer vs transient/rate-limited JWKS failures, keeps audience-refresh cache updates eviction-safe, rejects issuer inputs that normalize to empty values, and throttles repeated expected-audience refresh attempts after refresh errors to avoid per-request retry storms. `bgctl update` now uses command-scoped request contexts with an explicit HTTP timeout, removes partially extracted binaries on extraction errors, and propagates non-EOF probe-read errors during size-limited archive extraction. `MarshalIdentityProviderToJSON` now emits a sanitized payload that omits runtime secrets (for example client secrets and service-account tokens)
