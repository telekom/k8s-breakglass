...
- **MailProvider STARTTLS enforcement**: SMTP providers now fail closed when `disableTLS` is false and the server does not advertise or complete STARTTLS. Plaintext SMTP remains available only through explicit `spec.smtp.disableTLS: true` for development/test relays such as MailHog.
