...
- **Frontend: stable v-for key in BreakglassSessionReview** ([#535](https://github.com/telekom/k8s-breakglass/issues/535)): Use `metadata.name` as the primary `v-for` key with `session-${index}` as a last-resort fallback, preventing incorrect component reuse during filtering when session names are available
