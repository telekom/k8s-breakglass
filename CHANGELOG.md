...
- **bgctl local secret file permissions**: `bgctl` now tightens existing config and file-backed token cache files to owner-only permissions (`0600`) before writing inline OIDC client secrets or cached tokens, preventing permissive pre-created files from remaining group/world-readable.
