...
- **Session activity tracking** ([#314](https://github.com/telekom/k8s-breakglass/issues/314)): Added `lastActivityAt` and `accessCount` fields to `BreakglassSessionStatus`. The authorization webhook now records the timestamp and count of each successful session-based authorization. Activity updates are performed asynchronously to avoid blocking webhook responses. New Prometheus metrics: `breakglass_session_activity_total`, `breakglass_session_last_activity_timestamp`, and `breakglass_session_activity_update_errors_total`.
