...
- **CEL validation expressions on CRDs**: Added `x-kubernetes-validations` (CEL) rules to `BreakglassEscalation`, `DenyPolicy`, and `IdentityProvider` CRDs for compile-time cross-field validation enforced by the API server. Rules cover: blockSelfApproval requires approver groups, allowedIdentityProviders mutual exclusivity, SessionLimitsOverride unlimited conflicts, PodSecurityOverrides requireApproval requires approvers, DenyPolicy requires at least one rule, and IdentityProvider keycloak config required when groupSyncProvider is Keycloak (resolves #313)
