...
// waitForMailHogMessageWithBody waits for an email that matches both subject and body content.
// This is more reliable than subject-only matching when multiple tests run concurrently.
func waitForMailHogMessageWithBody(t *testing.T, subjectContains, bodyContains string, timeout time.Duration) *MailHogMessage {
	t.Helper()
	...
	for time.Now().Before(deadline) {
		...
		for i := range messages {
			...
			// Check body content
			bodyMatches := strings.Contains(strings.ToLower(messages[i].Content.Body), strings.ToLower(bodyContains))
...
func TestNotificationOnSessionCreation(t *testing.T) {
	...
	t.Run("SessionCreationEmailSent", func(t *testing.T) {
		...
		// Use body check to ensure we find the right email for this specific test
		msg := waitForMailHogMessageWithBody(t, escalation.Spec.EscalatedGroup, escalation.Spec.EscalatedGroup, helpers.WaitForStateTimeout)
		if msg == nil {
			// Try waiting for "pending" which is common in notification subjects, but still check body
			msg = waitForMailHogMessageWithBody(t, "pending", escalation.Spec.EscalatedGroup, 15*time.Second)
		}
...
func TestNotificationWithGroupApprovers(t *testing.T) {
	...
	t.Run("GroupApproverNotificationSent", func(t *testing.T) {
		...
		// Wait a moment for group sync to complete before checking for emails
		time.Sleep(2 * time.Second)

		// Use escalatedGroup in body check to ensure we find the right email for this specific test
		// This prevents interference from emails generated by other concurrent tests
		msg := waitForMailHogMessageWithBody(t, escalation.Spec.EscalatedGroup, escalation.Spec.EscalatedGroup, helpers.WaitForStateTimeout)
		if msg == nil {
			// Try waiting for "pending" as a fallback, but still check body for our session
			msg = waitForMailHogMessageWithBody(t, "pending", escalation.Spec.EscalatedGroup, 15*time.Second)
		}
		...
		// Verify that the email was sent to a valid recipient
		// The group sync may resolve to any member of the approver group
		// We just need to verify the email was delivered to someone
		recipientFound := false
		for _, to := range msg.To {
			fullEmail := to.Mailbox + "@" + to.Domain
			t.Logf("Email recipient: %s", fullEmail)
			// Accept any email recipient as valid - the important thing is that
			// group sync resolved the group to at least one email address
			if fullEmail != "" {
				recipientFound = true
			}
		}
...
func TestNotificationOnSessionApproval(t *testing.T) {
	...
	t.Run("ApprovalEmailSent", func(t *testing.T) {
		// Use body check with escalatedGroup to ensure we find the right email
		msg := waitForMailHogMessageWithBody(t, "approved", escalation.Spec.EscalatedGroup, helpers.WaitForStateTimeout)
		if msg == nil {
			// Also try looking for "active" since approved sessions become active
			msg = waitForMailHogMessageWithBody(t, "active", escalation.Spec.EscalatedGroup, 15*time.Second)
		}
...
func TestNotificationOnSessionRejection(t *testing.T) {
	...
	t.Run("RejectionEmailSent", func(t *testing.T) {
		// Use body check with escalatedGroup to ensure we find the right email
		msg := waitForMailHogMessageWithBody(t, "rejected", escalation.Spec.EscalatedGroup, helpers.WaitForStateTimeout)
