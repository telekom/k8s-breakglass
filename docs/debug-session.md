...
- The webhook uses indexed cached session discovery for normal requests and retries discovery through the live API reader when that cache has not observed a newly active session yet. The final session, expiry, participant, and Pod UID checks still use live state and fail closed on any mismatch.
