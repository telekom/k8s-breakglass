...
**RBAC requirements**: TOFU requires the controller to have `create` and `patch` permissions on Secrets in the target namespace. The persisted Secret is managed via Server-Side Apply (SSA) with the labels `app.kubernetes.io/managed-by: breakglass` and `breakglass.t-caas.telekom.com/tofu-ca: "true"`, and the annotation `breakglass.t-caas.telekom.com/tofu-timestamp` recording the bootstrap time. To audit pinned-CA Secrets, select on the label: `kubectl get secrets -A -l breakglass.t-caas.telekom.com/tofu-ca=true`.
