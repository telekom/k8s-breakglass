...
**RBAC requirements**: TOFU requires the controller to have `get`, `create` and `update` permissions on Secrets in the target namespace. The pin is written with optimistic concurrency (a `resourceVersion`-guarded update, retried on conflict) so that a CA which contradicts an existing pin can never overwrite it, even if a competing replica pins one mid-write. The persisted Secret carries the labels `app.kubernetes.io/managed-by: breakglass` and `breakglass.t-caas.telekom.com/tofu-ca: "true"`, and the annotation `breakglass.t-caas.telekom.com/tofu-timestamp` recording the bootstrap time. To audit pinned-CA Secrets, select on the label: `kubectl get secrets -A -l breakglass.t-caas.telekom.com/tofu-ca=true`.
