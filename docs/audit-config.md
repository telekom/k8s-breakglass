...
Before a DebugSession template snapshot exists, API, controller and cleanup-routine lifecycle audit events use the
...
Expiry and approval-timeout events apply the same policy regardless of whether
the controller or periodic cleanup routine wins the status update.
