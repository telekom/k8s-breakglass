...
status:
  ...
  # Activity tracking (updated by webhook via SSA)
...
These fields are updated by the authorization webhook's `ActivityTracker` via Server-Side Apply (dedicated `activity-tracker` field manager). They record when and how often a session is exercised.
...
Used as the primary baseline for idle timeout calculation. If not set, the controller falls back to `actualStartTime` > `approvedAt` > `creationTimestamp`.
