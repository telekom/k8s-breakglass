...
| `network-diagnostics` — [`network-debug`](../utils/network-debug/README.md) | Public catalogue profile: report/bounded-capture-only pod-network diagnostics with the image's fixed limits. It does not authorize host namespaces or full `pwru` tracing. | No public catalogue trace profile. Full host-network/host-PID `pwru` tracing is an image capability only and requires a separate custom `DebugSessionTemplate`/profile with independent privilege review and approval plus Linux prerequisites. Controller-mediated selected-pod capture remains bounded and separately reviewed; `pwru` is never a privileged fallback. | Grant only the required network capabilities. Delete capture pods and private evidence on completion; verify the ephemeral container and target pod are unchanged except for the approved injection. |
...
| `dump-access` — [`dump-reader`](../utils/images/dump-reader/README.md) | Bounded `inspect`, `checksum`, or `copy` of one approved regular file from the read-only `/input` mount. | No upstream advanced profile; providers may define a separately reviewed artifact handoff. | Mount the approved source read-only at `/input` and a separate writable `/output` volume; keep the non-root image, no capabilities, no host namespaces, and `allowExec: false`. The copy is size-bounded and never overwrites an existing output; remove the Job and retained output according to the incident policy. |
| `cluster-validation` — [`cluster-validator`](../utils/cluster-validator/README.md) | One-shot `--mode one-time` readiness report using only read-only Kubernetes APIs. | `--mode post-upgrade` is a separate one-shot check with the same report contract; it must not become a long-lived workload. | Enable only with a dedicated ServiceAccount and reviewed read-only RBAC for nodes, namespaces, pods, and discovery; explicitly opt in to the service-account token and mount writable `/reports` only when a file report is required. Keep the Job non-interactive and remove the Job and report artifacts through the normal retention policy. |
