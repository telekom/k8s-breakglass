...
cleanup() {
	...
	if [ "$KUBESTR_NAMESPACE_CREATED" = true ] && command -v kubectl >/dev/null 2>&1; then
		kubectl delete namespace "$KUBESTR_NAMESPACE" --ignore-not-found --wait=false >/dev/null 2>&1
	fi
...
for resolver_tool in dig host nslookup; do
	exec_in "$resolver_tool" "$CONTAINER" | grep -E '[0-9a-fA-F:.]+' >/dev/null || requirement "$resolver_tool Docker-DNS check failed"
done
...
for tool in curl dig host nslookup nc ping tracepath traceroute mtr ip ss tcpdump ethtool kubestr pwru; do
	printf '%s\n' "$tools_output" | grep -E "^${tool}[[:space:]]+installed$" >/dev/null || requirement "$tool is not installed according to net-debug tools"
done
...
exec_in tcpdump -nn -r /work/capture.pcap 'tcp port 18080' 2>/dev/null | grep -F 'IP' >/dev/null || requirement "captured pcap could not be read"

printf '%s\n' 'Checking pwru packet events on an ephemeral privileged namespace' >&2
btf_check=$(docker run --rm --privileged --network "container:$CONTAINER" "$IMAGE" sh -c \
	'test -r /sys/kernel/btf/vmlinux && test -e /sys/kernel/security && echo ready' 2>&1) || \
	requirement "pwru requires a Linux runner with readable /sys/kernel/btf/vmlinux, /sys/kernel/security, and privileged BPF access; use the dedicated compatible runner job"
test "$btf_check" = ready || requirement "pwru kernel prerequisites were not available"
docker run --detach --name "$PWRU_CONTAINER" --privileged --network "container:$CONTAINER" \
	"$IMAGE" pwru --output-limit-lines 1 --output-file /work/pwru.log --timestamp none \
